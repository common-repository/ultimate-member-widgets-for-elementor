...
use Elementor\Core\Kits\Documents\Tabs\Global_Typography;
	...
	class UM_ELEMENTOR_LIST_MODULE extends Widget_Base {
		...
		protected function render() {
		    ...
		    $args = $this->prepare_query_args($settings);

		    $members_query = new \WP_User_Query($args);
		    $users = (array)$members_query->get_results();

		    if (!empty($users)) {
		        $this->render_member_items($users, $settings);
		    } else {
		        esc_html_e('Not Found', 'um-elementor');
		    }
		}

		protected function prepare_query_args($settings) {
		...
		protected function render_member_items($users, $settings) {
		    ...
		    <div class="um-team-carousel-wrapper grid grid-cols-1 md:grid-cols-<?php echo esc_attr($column); ?>">
		        ...
		        foreach ($users as $user_id) {
			    	...
			    	if( get_user_meta( $user_id, 'hide_in_members', true ) && get_user_meta( $user_id, 'hide_in_members', true )[0] == "Yes"){
			    		return;
			    	}			    	
		            ...
		            if ($layout == 1) {
		                $this->render_member_layout_1($settings, $hover_animate, $align, $user_id);
		            }

		            if ($layout == 2) {
		                $this->render_member_layout_2($settings, $hover_animate, $align, $user_id);
		            }
		        }
		        ?>
		    </div>
		...
		protected function render_member_layout_1($settings, $hover_animate, $align, $user_id) {
		            ...
		            <div class="um-elementor-member-position">
		                <?php if ($settings['um_elementor_show_member_name'] == 1) : ?>
		                    <div class="um-elementor-member-name <?php echo esc_attr($align); ?>">
		                        <p>
		                            <a title="<?php echo esc_attr(um_user('first_name')); ?>"
		                               href="<?php echo esc_url(um_user_profile_url()); ?>">
		...
		protected function render_member_layout_2($settings, $hover_animate, $align, $user_id) {
		    ?>
		    <div class="um-item-user elementor-animation-<?php echo esc_attr($hover_animate); ?>">
		        <div class="um-widget-member-holder">
		            <div class="user-layout-spaced grid">
		                <div class="user-layout-spaced-one um-elementor-member-image <?php echo esc_attr($align); ?>">
		                    <a title="<?php echo um_user('display_name') ?>"
